...
function contextualPerson(person: Member, members: Member[]): AIGenealogyPerson {
  ...
  const parents = family.parents.map((relative) => personFact(relative, members));
  const spouses = family.spouses.map((relative) => personFact(relative, members));
  const children = family.children.map((relative) => personFact(relative, members));
  const siblings = family.siblings.map((relative) => personFact(relative, members));
  return {
    ...
    parentRelations: parentRelationsOf(person).flatMap((relation) => {
      ...
      return parent ? [{ person: personFact(parent, members), kind: relation.kind }] : [];
    }),
    childRelations: family.children.flatMap((child) => {
      const relation = parentRelationsOf(child).find((link) => link.parentId === person.id);
      return relation ? [{ person: personFact(child, members), kind: relation.kind }] : [];
    }),
    siblingRelations: family.siblings.map((sibling) => {
      ...
      return {
        ...
        ...(result?.relationshipCode ? { relationshipCode: result.relationshipCode } : {}),
...
export function buildGenealogyContext({
  ...
  const relationship = ambiguities.length || people.length < 2
    ? undefined
